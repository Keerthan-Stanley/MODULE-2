...
This section walks through running GLS on a synthesized netlist, and deliberately reproducing a mismatch caused by blocking-assignment misuse, to see the theory in action.
...
**Conclusion:** The RTL and gate-level simulations disagree — a textbook synthesis-simulation mismatch caused by an incomplete sensitivity list. Fix: use `always @(*)` instead of `always @(sel)`.
...
gtkwave sim_b.vcd
```
**What to check:** Compare this waveform against the non-blocking version from Lab 3 — `q2` will now track `d` with only **1 cycle of delay** instead of 2, even though the code visually looks like a 2-stage shift register.

**Follow-up:** Synthesize `shift_blocking.v` with Yosys and run GLS on it. Since real flip-flops physically cannot behave the way the blocking-assignment simulation suggests, the gate-level simulation will show the **correct 2-stage delay** — directly demonstrating a synthesis-simulation mismatch caused by blocking-statement misuse in sequential logic.
